...
if(typeof window != 'undefined'){
	...
	(function(){
		...
		const n = navigator;
		const dua = n.userAgent;
		const dav = n.appVersion;
		const tv = parseFloat(dav);
		...
		const oldLoadUri = dojo._loadUri;
		dojo._loadUri = function(uri, cb){
			const handleLocal = ["file:", "chrome:", "resource:"].some(function(prefix){
				return String(uri).indexOf(prefix) == 0;
			});
			if(handleLocal){
				// see:
				//		http://developer.mozilla.org/en/mozIJSSubScriptLoader
				const l = Components.classes["@mozilla.org/moz/jssubscript-loader;1"]
					.getService(Components.interfaces.mozIJSSubScriptLoader);
				const value = l.loadSubScript(uri, dojo.global);
				if(cb){ cb(value); }
				return true;
			}else{
		...
		dojo._getText = function(/*URI*/ uri, /*Boolean*/ fail_ok){
			...
			if(handleLocal){
				...
				const channel = ioService.newChannel(uri, null, null);
				const input = channel.open();
				scriptableStream.init(input);
				const str = scriptableStream.read(input.available());
				...
				try{
					...
					if(!dojo._isDocumentOk(http)){
						const err = Error(`Unable to load ${uri} status:${http.status}`);
		...
		dojo.windowUnloaded = function(){
			...
			const mll = dojo._windowUnloaders;
			while(mll.length){
				(mll.pop())();
			}
		};
		...
		dojo.pushContext = function(/*Object|String?*/g, /*MDocumentElement?*/d){
			...
			const old = [dojo.global, dojo.doc];
			contexts.push(old);
			let n;
			if(!g && !d){
		...
		dojo._inContext = function(g, d, f){
			const a = dojo._toArray(arguments);
			f = a.pop();
			if(a.length == 1){
				d = null;
			}
	...
	dojo._loadInit = function(e){
		...
		const type = (e && e.type) ? e.type.toLowerCase() : "load";
...
(function(){
	const mp = dojo.config["modulePaths"];
	if(mp){
		for(const param in mp){
...
if(dojo.config.isDebug){
	...
	console.log = function(m){
		const s = Components.classes["@mozilla.org/consoleservice;1"].getService(
