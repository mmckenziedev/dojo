...
dojo._spidermonkeyCurrentFile = function(depth){
	...
	const matches = s.match(/[^@]*\.js/gi);
	if(!matches){
		throw Error(`could not parse stack string: '${s}'`);
	}
	const fname = (typeof depth != 'undefined' && depth) ? matches[depth + 1] : matches[matches.length - 1];
	if(!fname){
...
if(dojo.config["modulePaths"]){
	for(const param in dojo.config["modulePaths"]){
